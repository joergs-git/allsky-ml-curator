...
> **Version:** 0.8.8 — embedding warmer respects Night-only / Day-only. Previously it Vision-embedded every frame (incl. daytime frames the matrix and trainer ignore); now it reads AppSettings and filters at the SQL level, saving ~61 % of the work on a night-focused library. Built on 0.8.7's multi-folder ingest: ⌘O picker accepts multi-selection; ingest sheet holds a folder queue with per-item remove and Clear. Single pass across all folders under one isRunning window, one cancelToken, shared counters. Built on 0.8.6's soft-exclude for Delete. Backspace flips `images.isExcluded=1` instead of hard-deleting the row; re-ingest honours the existing filePath and no longer resurrects excluded frames. A new rating filter "Only excluded (trash)" surfaces the pile so the curator can restore mistakes — same Backspace inverts to restore in that view. Built on 0.8.5's "How to start" button in the toolbar opens a floating workflow guide (two-phase: bootstrap once, daily loop). Keeps the idiomatic flow discoverable so new sessions don't re-sweep every iteration or expect auto-rate to touch human labels. Built on 0.8.4's sweep got a camera-scope picker (segmented `Colour` / `Mono` next to Run Sweep) so the autopilot actually tunes the per-camera MLP it's pointed at instead of mixing both back together. Weather-ingest sheet gained "Seed window from current class-2 labels": reads IQR of current class-2 sky-temps for the selected camera, pads ±0.5 °C, widens the date window to all-time — targeted hunt for more class-2 frames. Side panel still surfaces per-camera classifier stats from 0.8.3 (COLOUR / MONO pill). Built on 0.8.2's per-camera MLP split (colour OSC + monochrome train as two separate classifiers, ⌘T trains both in one pass, `predict(image:)` routes by camera, `model_versions.cameraScope` carries the split). 0.8.1's 3-class (unsuitable / partial / suitable) collapse underpins both. Rate → retrain → inspect-mismatches → auto-rate → targeted re-ingest → remove-wrong-ones workflow is end-to-end. Training runs on a detached task so the UI stays responsive during ⌘T; night-only filter soft-hides day / twilight frames from matrix + training; rated tiles where the model disagrees get a dashed orange border + predicted-class badge and can be filtered to as a dedicated rating-filter view; embedding warmer is a start/stop toggle on the Embeddings chip; per-tile moon + auto-reflection risk icons (moon threshold tunable in Preferences, default 30°); class weighting is a per-class boost vector.
